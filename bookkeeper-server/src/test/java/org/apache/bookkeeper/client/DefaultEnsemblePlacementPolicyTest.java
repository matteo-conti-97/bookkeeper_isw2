...
public class DefaultEnsemblePlacementPolicyTest {
    ...
    public static Collection<Object[]> getTestParameters() {
        return Arrays.asList(new Object[][]{ // ensebmleSize, writeQuorumSize, ackQuorumSize, customMetadata, excludeBookieSetType, isWeighted, expected
                ...
                //12 - ensembleSize 0, writeQuorumSize 0, ackQuorumSize 0, no metadata, empty set, true -> Ensemble with no bookies
    ...
    public static Collection<Object[]> getTestParameters() {
        return Arrays.asList(new Object[][]{ // ensebmleSize, writeQuorumSize, ackQuorumSize, customMetadata, isWeighted, excludeBookies, knownBookies, weightedRandomSelection expected
                //0 - ensembleSize -1, writeQuorumSize -1, ackQuorumSize -1, no metadata, false, empty set -> IllegalArgumentException for the -1
                {-1, -1, -1, new HashMap<>(), false, new HashSet<>(), new IllegalArgumentException()},
                //1 - ensembleSize 0, writeQuorumSize 0, ackQuorumSize 0, no metadata, false, empty set -> Valid ensembleSize/writeQuorumSize/ackQuorumSize but 0 -> Ensemble with no bookies
                {0, 0, 0, new HashMap<>(), false, new HashSet<>(), 0},
                //2 - ensembleSize 1, writeQuorumSize 1, ackQuorumSize 1, no metadata, false, empty set -> Ensemble with 1 bookie
                {1, 1, 1, new HashMap<>(), false, new HashSet<>(), 1},
                //3 - ensembleSize 1, writeQuorumSize 1, ackQuorumSize 1, no metadata, false, null set -> NullPointerException
                {1, 1, 1, new HashMap<>(), false, null, new NullPointerException()},
                //4 - ensembleSize 1, writeQuorumSize 1, ackQuorumSize 1, no metadata, false, set con un bookie -> Ensemble with 1 bookie
                {1, 1, 1, new HashMap<>(), false, createDummyHashSet(1), 1},
                //5 - ensembleSize 1, writeQuorumSize 1, ackQuorumSize 2, no metadata, false, empty set -> Illegal argument exception ackQuorumSize > writeQuorumSize -> Bug lui crea tranquillamente l'insieme con 1 bookie
                //{1, 1, 2, new HashMap<>(), false, new HashSet<>(), new IllegalArgumentException()},
                //6 - ensembleSize 1, writeQuorumSize 1, ackQuorumSize 2, no metadata, false, empty set -> Illegal argument exception writeQuorumSize > ensembleSize -> Bug lui crea tranquillamente l'insieme con 1 bookie
                //{1, 2, 1, new HashMap<>(), false, new HashSet<>(), new IllegalArgumentException()},
                //7 - ensembleSize 1, writeQuorumSize 0, ackQuorumSize 0, no metadata, false, empty set -> Ensemble with 1 bookie
                {1, 0, 0, new HashMap<>(), false, new HashSet<>(), 1},
                //8 - ensembleSize 1, writeQuorumSize 1, ackQuorumSize 0, no metadata, false, empty set -> Ensemble with 1 bookie
                {1, 1, 0, new HashMap<>(), false, new HashSet<>(), 1},
                //9 - ensembleSize 1, writeQuorumSize 1, ackQuorumSize 0, null metadata, false, empty set -> Ensemble with 1 bookie -> no use metadata
                {1, 1, 1, null, false, new HashSet<>(), 1},
                //10 - ensembleSize 1, writeQuorumSize 1, ackQuorumSize 0, null metadata, false, empty set -> Ensemble with 1 bookie -> use metadata
                {1, 1, 1, ImmutableMap.of("testBookie", "TestMetadata".getBytes(StandardCharsets.UTF_8)), false, new HashSet<>(), 1},

                //EVO 1
                //11 - ensembleSize -1, writeQuorumSize -1, ackQuorumSize -1, no metadata, true, empty set -> IllegalArgumentException for the -1
                {-1, -1, -1, new HashMap<>(), true, new HashSet<>(), new IllegalArgumentException()},
                //12 - ensembleSize 0, writeQuorumSize 0, ackQuorumSize 0, no metadata, true, empty set -> Ensemble with no bookies
                {0, 0, 0, new HashMap<>(), true, new HashSet<>(), 0},
                //13 - ensembleSize 1, writeQuorumSize 1, ackQuorumSize 1, no metadata, true, empty set -> Ensemble with 1 bookie
                {1, 1, 1, new HashMap<>(), true, new HashSet<>(), 1},
                //14 - ensembleSize 1, writeQuorumSize 1, ackQuorumSize 1, no metadata, true, set con un bookie -> Ensemble with 1 bookie
                {1, 1, 1, new HashMap<>(), createDummyHashSet(1), true, 1},
                //15 - ensembleSize 1, writeQuorumSize 0, ackQuorumSize 0, no metadata, true, empty set -> Ensemble with 1 bookie
                {1, 0, 0, new HashMap<>(), true, new HashSet<>(), 1},
                //16 - ensembleSize 1, writeQuorumSize 1, ackQuorumSize 0, no metadata, true, empty set -> Ensemble with 1 bookie
                {1, 1, 0, new HashMap<>(), true, new HashSet<>(), 1},
        });
    }
    ...
    public void newEnsembleTest(){
        ...
        try {
            ...
            knownBookiesAttribute.set(esp, createDummyHashSet(7));
