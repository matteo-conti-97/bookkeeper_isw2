...
public class BufferedChannelWriteTest{
    ...
    public static Collection<Object[]> getTestParameters() throws FileNotFoundException {
        return Arrays.asList(new Object[][]{ //srcBuff, writeCapacity, fc, filename, expected
                //0 - srcBuff null, 1, fc empty file, empty file name -> Error null srcBuff -> NullPointerException
                {null, 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                BufferedChannelUtils.PATH_PREFIX +  BufferedChannelUtils.EMPTY_FILE_NAME), "rw").getChannel(),
                BufferedChannelUtils.EMPTY_FILE_NAME, new NullPointerException()},

                //1 - srcBuffDim=0, 1, fc empty file, empty file name -> Error srcBuffDim 0 -> Stringa vuota
                {Unpooled.buffer(0), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                BufferedChannelUtils.PATH_PREFIX +  BufferedChannelUtils.EMPTY_FILE_NAME), "rw").getChannel(),
                BufferedChannelUtils.EMPTY_FILE_NAME, ""},

                //2 - srcBuffDim=1 empty, 1, fc empty file, empty file name -> Ok -> Stringa vuota
                {Unpooled.buffer(1), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                BufferedChannelUtils.PATH_PREFIX +  BufferedChannelUtils.EMPTY_FILE_NAME), "rw").getChannel(),
                BufferedChannelUtils.EMPTY_FILE_NAME, ""},

                //3 - srcBuffDim=1 1Byte data, 1, fc empty file, empty file name -> Ok -> Stringa "a"
                {Unpooled.buffer(1).writeByte((byte) 'a'), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                BufferedChannelUtils.PATH_PREFIX +  BufferedChannelUtils.EMPTY_FILE_NAME), "rw").getChannel(),
                BufferedChannelUtils.EMPTY_FILE_NAME, "a"},

                //4 - arcBuffDim=1 1Byte data, 0, fc empty file, empty file name  file -> Error writeCapacity 0 -> -> Stringa vuota -> Entra in loop infinito è un bug
                //{Unpooled.buffer(1).writeByte((byte) 'a'), 0, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                //BufferedChannelUtils.PATH_PREFIX +   BufferedChannelUtils.EMPTY_FILE_NAME), "rw").getChannel(),
                //BufferedChannelUtils.EMPTY_FILE_NAME, ""},

                //5 - srcBuffDim=1 1Byte data, 1, null, null -> Error null fc -> NullPointerException
                {Unpooled.buffer(1).writeByte((byte) 'a'), 1, null, null, new NullPointerException()},

                //6 - srcBuffDim=1 1Byte data, 1, fc  file contenente 1 byte, !empty file name -> Ok -> Stringa "ca"
                {Unpooled.buffer(1).writeByte((byte) 'a'), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                BufferedChannelUtils.PATH_PREFIX +   BufferedChannelUtils.NON_EMPTY_FILE_NAME), "rw").getChannel(),
                BufferedChannelUtils.NON_EMPTY_FILE_NAME, "ca"}
        });
    } */
    ...
    public static Collection<Object[]> getEvolutedTestParameters() throws FileNotFoundException {
        return Arrays.asList(new Object[][]{ //srcBuff, writeCapacity, fc, unpersistedBytesBound, filename, expected
               ...
               //0 - srcBuff null, 1, fc empty  file, empty file name, unpersistedBytesBound 0  -> Error null srcBuff -> NullPointerException
                {null, 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                        BufferedChannelUtils.PATH_PREFIX +  BufferedChannelUtils.EMPTY_FILE_NAME), "rw").getChannel(),
                        0L, BufferedChannelUtils.EMPTY_FILE_NAME, new NullPointerException()},

                //1 - srcBuffDim=0, 1, fc empty  file, empty file name, unpersistedBytesBound 0  -> Error srcBuffDim 0 -> Stringa vuota
                {Unpooled.buffer(0), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                        BufferedChannelUtils.PATH_PREFIX +  BufferedChannelUtils.EMPTY_FILE_NAME), "rw").getChannel(),
                        0L, BufferedChannelUtils.EMPTY_FILE_NAME, ""},

                //2 - srcBuffDim=1 empty, 1, fc empty  file, empty file name, unpersistedBytesBound 0  -> Ok -> Stringa vuota
                {Unpooled.buffer(1), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                        BufferedChannelUtils.PATH_PREFIX +  BufferedChannelUtils.EMPTY_FILE_NAME), "rw").getChannel(),
                        0L, BufferedChannelUtils.EMPTY_FILE_NAME, ""},

                //3 - srcBuffDim=1 1Byte data, 1, fc empty  file, empty file name, unpersistedBytesBound 0  -> Ok -> Stringa "a"
                {Unpooled.buffer(1).writeByte((byte) 'a'), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                        BufferedChannelUtils.PATH_PREFIX +  BufferedChannelUtils.EMPTY_FILE_NAME), "rw").getChannel(),
                        0L, BufferedChannelUtils.EMPTY_FILE_NAME, "a"},

                //4 - arcBuffDim=1 1Byte data, 0, fc empty  file, empty file name, unpersistedBytesBound 0  -> Error writeCapacity 0 -> Stringa vuota -> Entra in loop infinito è un bug
                //{Unpooled.buffer(1).writeByte((byte) 'a'), 0, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                // BufferedChannelUtils.PATH_PREFIX +   BufferedChannelUtils.EMPTY_EXISTING_FILE_NAME), "rw").getChannel(),
                // 0L, BufferedChannelUtils.EMPTY_EXISTING_FILE_NAME, ""},

                //5 - srcBuffDim=1 1Byte data, 1, null, null-> Error null fc, unpersistedBytesBound 0  -> NullPointerException
                {Unpooled.buffer(1).writeByte((byte) 'a'), 1, null, 0L, null, new NullPointerException()},

                //6 - srcBuffDim=1 1Byte data, 1, fc  file contenente 1 byte, !empty file name, unpersistedBytesBound 0 -> Ok -> Stringa "ca"
                {Unpooled.buffer(1).writeByte((byte) 'a'), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                        BufferedChannelUtils.PATH_PREFIX +   BufferedChannelUtils.NON_EMPTY_FILE_NAME), "rw").getChannel(),
                        0L, BufferedChannelUtils.NON_EMPTY_FILE_NAME, "ca"},

                // EVO 1 -> Casi di test aggiunti a seguito dell'evoluzione 1 dei test, aggiunto il parametro unpersistedBytesBound
                //7 - srcBuffDim=1 1Byte data, 1, fc  file contenente 1 byte, !empty file name, unpersistedBytesBound 1 -> Ok -> Stringa "ca"
                {Unpooled.buffer(1).writeByte((byte) 'a'), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                        BufferedChannelUtils.PATH_PREFIX +   BufferedChannelUtils.NON_EMPTY_FILE_NAME), "rw").getChannel(),
                        1L, BufferedChannelUtils.NON_EMPTY_FILE_NAME, "ca"},

                //8 - srcBuffDim=1 1Byte data, 1, fc empty  file, empty file name, unpersistedBytesBound 1 -> Ok -> Stringa "a"
                {Unpooled.buffer(1).writeByte((byte) 'a'), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                        BufferedChannelUtils.PATH_PREFIX +  BufferedChannelUtils.EMPTY_FILE_NAME), "rw").getChannel(),
                        1L, BufferedChannelUtils.EMPTY_FILE_NAME, "a"},

                // EVO 2 -> Casi di test aggiunti a seguito dell'evoluzione 2 dei test, vengono dati in input src contenenti 2byte
                //9 - srcBuffDim=2 2Byte data, 1, fc empty  file, empty file name, unpersistedBytesBound 0 -> Ok -> Stringa "ab"
                {Unpooled.buffer(2).writeBytes(new byte[] {(byte)'a', (byte)'b'}), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                        BufferedChannelUtils.PATH_PREFIX +  BufferedChannelUtils.EMPTY_FILE_NAME), "rw").getChannel(),
                        0L, BufferedChannelUtils.EMPTY_FILE_NAME, "ab"},

                //10 - srcBuffDim=2 2Byte data, 1, fc  file contenente 1 byte, empty file name, unpersistedBytesBound 0 -> Ok -> Stringa "cab"
                {Unpooled.buffer(2).writeBytes(new byte[] {(byte)'a', (byte)'b'}), 1, new RandomAccessFile(new File(BufferedChannelUtils.ROOT_DIR_PATH,
                        BufferedChannelUtils.PATH_PREFIX +  BufferedChannelUtils.NON_EMPTY_FILE_NAME), "rw").getChannel(),
